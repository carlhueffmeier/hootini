...
// For development: import configuration options from 'variables.env'
if (process.env) require('dotenv').config({ path: 'variables.env' });
...
const server = app.listen(app.get('port'), () => {
  const playgroundUrl = `http://localhost:${server.address().port}` + apolloServer.graphqlPath;
